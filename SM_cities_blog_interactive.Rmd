---
title: "A Class of Their Own: Housing Characteristics of Small and Mid-Size Cities"
...
The analysis presented here appears in the blog post [_Housing Characteristics of Small and Mid-Sized Cities
_](https://furmancenter.org/thestoop/entry/housing-characteristics-of-small-and-mid-sized-cities)
...
We utilize Bureau of Labor Statistics' Consumer Price Index (CPI) factor to normalize rents between 2000 and 2019. To access 2000 Decennial data, we use the US Census Bureau's API, which requires an access key.
```{r}
...
figure_1_data <- all_city_comparison_viz %>% filter(size_type == "small (50-100k)")

figure_1 <- ggplot(figure_1_data, aes(x = change_rounded, y = y, tooltip =scales::percent(change, .1), 
                       text = sprintf("%s<br>Change in homeownership rate, 2000- 2019: %s",
                                      geo_name,
                                      scales::percent(change, .1)) 
      )
                         ) +
...
#Export as HTML
saveWidget(plot_interactive_figure_1, "plot_interactive_figure_1.html")
```
...
figure_2 <- ggplot(data = chart_data_rent, aes(x = variable, y = value, label = value, tooltip = value, group = size_type,
                                                text = sprintf("City size category: %s<br>%s: %s", 
                                                size_type, 
                                                str_sub(variable, end = -5),
                                                scales::dollar(value))
      )) +
...
##make the figure interactive 
plot_interactive_figure_2 <- ggplotly(figure_2, tooltip = 'text')%>%
 layout(
 legend = list(orientation = "h", yanchor = "top", y = 1.02),
 title = list(text = paste0('Weighted Median Gross Rent (in 2019 Dollars), 2000 and 2015-19',
                                    '<br>',
                                    '<sup>',
                                    'Small-Mid Sized vs. Large Cities',
                                    '</sup>')))

plot_interactive_figure_2

#Export as HTML
saveWidget(plot_interactive_figure_2, "plot_interactive_figure_2.html")
...
figure_3 <- ggplot(chart_data_burden, aes(x = variable, y = value, fill = size_type, label = scales::percent(value, .1), tooltip = scales::percent(value, .1),
                                          text = sprintf("City size category: %s<br>Household Income: %s<br>Share of Rental Households who are Housing Cost Burdened: %s", 
      size_type, variable, 
...
plot_interactive_figure_3 <- ggplotly(figure_3
                                      , tooltip = 'text')%>%
 layout(
   
 legend = list(orientation = "v", title = ""),
 title = list(text = paste0('Share of Rental Households who are Housing Cost Burdened by Income, 2019',
                                    '<br>',
                                    '<sup>',
                                    'Small-Mid Sized vs. Large Cities',
                                    '</sup>')))

plot_interactive_figure_3

#Export as HTML
saveWidget(plot_interactive_figure_3, "plot_interactive_figure_3.html")
```
...
figure_4 <- ggplot(chart_homeownership_2019, aes(size_type, value, label = scales::percent(value, .1), fill = factor(variable),
                                                 tooltip = scales::percent(value, .1),
                                          text = sprintf("City size category: %s<br>Race and ethnicty: %s<br>Homeownership rate, 2019: %s", 
      size_type, variable, 
      scales::percent(value, accuracy = .1)) )) +
  geom_bar(stat = "identity", width = .9, position = "dodge") +
  # geom_text(
...
##make the figure interactive 
plot_interactive_figure_4 <- ggplotly(figure_4, tooltip = 'text')%>%
 layout(
 legend = list(title = ""),
 title = list(text = paste0('Homeownership Rates by Race and Ethnicity, 2019',
                                    '<br>',
                                    '<sup>',
                                    'Small-Mid Sized vs. Large Cities',
                                    '</sup>')))

plot_interactive_figure_4

#Export as HTML
saveWidget(plot_interactive_figure_4, "plot_interactive_figure_4.html")
