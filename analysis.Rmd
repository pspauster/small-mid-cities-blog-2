...
saveRDS(pha_geo, file = "pha_service_areas.rds")
...
mtw_phas <- "https://www.hud.gov/program_offices/public_indian_housing/programs/ph/mtw/mtwagencies" %>% 
  ...
  mutate(state_ab = str_sub(mtw_agencies_and_state_location, start = -3, end = -2),
         ...
         mtw_flag = 1,
         cohort = initial_or_expansion_agency) %>% 
  select(state_ab, pha_name, mtw_flag, cohort) %>% 
...
psh_phas_mtw <- left_join(psh_phas_clean, mtw_phas, by = c("state_ab", "pha_name")) %>% 
  ...
  cohort = case_when( #these refuse to match or are two different ha
    pha_name == "housing authority of baltimore city" ~ "Initial",
    pha_name == "housing authority of champaign county" ~ "Initial",
    pha_name == "holyoke housing authority" & state_ab == "MA"~ "Initial",
    pha_name == "santa clara county housing authority" ~ "Initial",
    pha_name == "housing authority of the city of san jose" ~"Initial",
    T ~ cohort
  ))
...
psh_phas_wide <- psh_phas_mtw %>% 
  ...
  pivot_wider(id_cols = c(PARTICIPANT_CODE, pha_name, mtw_flag, cohort, state_ab),
...
mtw_part_codes <- psh_phas_mtw %>% 
  filter(mtw_flag == 1) %>% 
  select(pha_name, PARTICIPANT_CODE, mtw_flag, cohort) %>% 
  distinct() %>% 
  janitor::clean_names()

mtw_part_codes %>% count(cohort)
...
mtw_serving %>% 
  distinct(participant_code, pha_level, cohort) %>% 
  group_by(pha_level, cohort) %>% 
  count()
...
mtw_serving %>% 
  ...
  group_by(target_size, cohort) %>% 
  count()

#how many mtw phas serve a city
mtw_serving %>% 
  group_by(pha_name, cohort) %>% 
...
```{r}
small_cohort <- psh_phas_wide %>% 
  filter(cohort == "MTW Flexibility for Smaller PHAs Cohort")
...
mtw_demo <- psh_phas_wide %>%
  #filter(mtw_flag == 1) %>% 
  select(participant_code, mtw_flag, cohort,
  ...
  mutate(mtw_cat = factor(
    ifelse(is.na(cohort), "Not MTW", cohort), levels = c("Initial", "Stepped and Tiered Rent Cohort", "MTW Flexibility for Smaller PHAs Cohort", "Not MTW")
    ),
...
psh_phas_wide %>%
  ...
  mutate(hhs_bracket = factor(case_when(
    total_units_pha_1_summary_of_all_hud_programs < 250 ~ "Less than 250",
    total_units_pha_1_summary_of_all_hud_programs >= 250 & total_units_pha_1_summary_of_all_hud_programs < 500 ~ "250-500",
    total_units_pha_1_summary_of_all_hud_programs >= 500 & total_units_pha_1_summary_of_all_hud_programs < 750 ~ "500-750",
    total_units_pha_1_summary_of_all_hud_programs >= 750 & total_units_pha_1_summary_of_all_hud_programs < 1000 ~ "750-1,000",
    total_units_pha_1_summary_of_all_hud_programs >= 1000 ~ "1,000 or more"
  ),levels = c(
    "Less than 250",
    "250-500",
    "500-750",
...
ggplot()+
  ...
  labs(title = "All small cohort agencies serve less than 1000 hh",
...
psh_phas_wide %>%
  ...
  select(participant_code, mtw_flag, cohort,
         total_units_pha_1_summary_of_all_hud_programs) %>% 
  mutate(hhs_bracket = factor(case_when(
    total_units_pha_1_summary_of_all_hud_programs < 500 ~ "Less than 500",
    total_units_pha_1_summary_of_all_hud_programs >= 500 & total_units_pha_1_summary_of_all_hud_programs < 1000 ~ "500-1,000",
  ...
  count(hhs_bracket) %>% 
ggplot()+
  geom_col(mapping = aes(x = hhs_bracket, y = n, fill = cohort))+
  labs(title = "All small cohort agencies serve less than 1000 hh",
       subtitle = "Distribution of # of households served",
       y = "Number of PHAs",
       x = "# of households served",
