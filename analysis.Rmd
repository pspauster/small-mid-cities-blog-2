...
mix_hh_served %>% 
  ...
  geom_text(aes(x = "", y = ypos, label = sum, color = size_type))+
...
census_psh_places %>% 
  select(pct_disabled_all_place_1_summary_of_all_hud_programs, size_type) %>% 
  group_by(size_type) %>% 
  summarize(avg_disabled_share = mean(pct_disabled_all_place_1_summary_of_all_hud_programs/100, na.rm = TRUE)) %>% 
  mutate(size_type = factor(size_type, levels = c("small (50-100k)", "small-mid (100-150k)",  "mid (150-500k)", "large (500k +)"))) %>% 
  ggplot()+
  geom_col(mapping = aes(x = size_type, y = avg_disabled_share)) +
  scale_y_continuous(expand = expand_scale(mult = c(0, 0.1)), labels = (scales::percent)) +
  labs(title = "Average share of households with a disabled person is consistent across city sizes",
       subtitle = "Average share of households with a disabled member by city type",
        x = NULL,
       y = NULL) +  
  remove_ticks() +
  guides(color = FALSE)
...
check on... 1/5/21
-disability seems low
-add average housing assistance amount
-how many state phas are there
-how many w/no vouchers or no public housing
-proportion that live in low/med/high  pov
-how does the data define high poverty
